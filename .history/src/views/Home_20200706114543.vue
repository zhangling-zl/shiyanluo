<template>
  <div>
    ...
    <navHome></navHome>
    <!--banner广告图-->
    <bannerHome :banner="banner" v-if="banner.length>0" :course="course"></bannerHome>
    <!--楼+ 实战课程-->
    ...
    <intensive :strengthen="strengthen" v-if="strengthen.length>0"></intensive>

    <buildingTitle>
      <template #big>学习路径 |</template>
      <template #small>精准而系统的学习路径，从0到大牛</template>
      <template #more>更多></template>
    </buildingTitle>
    <learningpath :paths="paths" v-if="paths.length>0"></learningpath>
...
import learningpath from "../components/home/learningpath"; //学习路径

import fonnts from "../components/home/fonnts"; //底部
...
export default {
  ...
  methods: {
    ...
    getData4() {
      axios
        .get("http://120.78.14.107/api/v2/index/paths/")
        .then(res => {
          if (res.data.length > 0) {
            this.paths = res.data;
            // console.log(res.data);
      ...
      axios
        ...
        .then(res => {
          if (res.data.length > 0) {
            ...
            res.data.map(item => {
              this.$set(item, "flag", false);//给对象增加原本没有的属性
            });
          }
          console.log(res.data);
        })
